...
### 효과적인 프롬프트 5요소
```
1. 역할 (Role)      → "React + Tailwind 전문 개발자로서"
2. 컨텍스트 (Context) → "기존 디자인 시스템 사용, 모바일 우선"
3. 작업 (Task)      → "로그인 폼 컴포넌트 만들어줘"
4. 요구사항 (Reqs)   → "이메일/비밀번호/기억하기/제출버튼 포함"
5. 출력형식 (Format) → "TypeScript + JSDoc 주석 포함"
```

### 프롬프트 템플릿
```
역할: {기술스택} 전문 프론트엔드 개발자로서
컨텍스트:
...
요구사항:
- 기존 컴포넌트 재사용 (Button, Card, Input 등)
- 4가지 상태 처리 (로딩/에러/빈/성공)
- 브랜드 색상만 사용 (brand-yellow, brand-cyan 등)
- 터치 타겟 최소 44x44px
...
```
1단계: 구조 우선
"버튼, 입력, 텍스트 위치만 잡아줘. 스타일은 기본만"

2단계: 기능 추가
"폼 검증, 제출 로직, 에러 표시 추가해줘"

3단계: 스타일 다듬기
"디자인 시스템 색상 적용하고 애니메이션 추가해줘"
```

### v0.dev 활용 워크플로우
```
1. v0.dev에서 프로토타입 생성
2. 코드 복사 후 프로젝트에 붙여넣기
3. 기존 디자인 시스템에 맞게 수정:
   - 색상 → brand-* 클래스로 교체
   - 버튼 → Button 컴포넌트로 교체
   - 카드 → Card 컴포넌트로 교체
4. 접근성 검수 (aria, label 등)
```

### AI 도구별 활용법
```
Claude/GPT      복잡한 로직, 아키텍처 설계, 코드 리뷰
v0.dev          빠른 UI 프로토타이핑, 레이아웃 구조
Figma AI       디자인 에셋, 아이콘, 일러스트
GitHub Copilot  자동완성, 반복 코드, 테스트
```

## 디자이너 없이 작업하기

### Copy, Don't Create 원칙
```
❌ "이 화면 디자인해줘"
✅ "Linear.app의 이슈 목록 레이아웃 참고해서 만들어줘"

좋은 레퍼런스 출처:
- Linear, Notion, Stripe → SaaS 대시보드
- Apple, Airbnb → 마케팅/랜딩
- Vercel, Supabase → 개발자 도구
```
...
## 컨텍스트
여러 색상의 버튼이 무분별하게 사용되고 있어 일관성 필요

## 결정
- Primary: brand-yellow (메인 CTA)
- Secondary: brand-purple (보조 액션)
- Danger: brand-pink (삭제/경고)
- Info: brand-cyan (정보성)

## 결과
모든 새 버튼은 이 4가지 중 하나를 사용해야 함
```
...
설치:
<script>clarity.init("프로젝트ID")</script>
```

### 디자인 시스템 체크리스트
```
□ 색상 팔레트 정의됨 (brand-* 클래스)
□ 타이포그래피 스케일 있음 (text-sm/base/lg/xl)
□ 간격 체계 일관됨 (p-4, gap-4, space-y-4)
□ 공통 컴포넌트 라이브러리 있음
□ 반응형 브레이크포인트 통일됨
□ 접근성 기준 명시됨
```
