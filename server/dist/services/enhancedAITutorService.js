...
exports.EnhancedAITutorService = void 0;
const openai_1 = require("../config/openai");
const AITutorSession_1 = require("../models/AITutorSession");
const LearningCoachSession_1 = require("../models/LearningCoachSession");
...
class EnhancedAITutorService {
     ...
     */
    static async startSession(userId, sessionType = AITutorSession_1.SessionType.GENERAL_QA) {
        // End any active sessions
        await AITutorSession_1.AITutorSession.updateMany({ userId: new mongoose_1.default.Types.ObjectId(userId), isActive: true }, { isActive: false, endedAt: new Date() });
        // Get user context
        ...
        if (profile) {
            ...
            if (recentPerformance.length > 0) {
                const recentAccuracy = recentPerformance.filter((p) => p.isCorrect).length / recentPerformance.length;
        ...
        // Create new session
        const session = await AITutorSession_1.AITutorSession.create({
            userId: new mongoose_1.default.Types.ObjectId(userId),
            sessionType,
            context,
            isActive: true,
        ...
        await session.addMessage(AITutorSession_1.MessageRole.SYSTEM, welcomeMessage);
        return session;
    }
    /**
     * Generate welcome message based on session type and user context
     */
    static async generateWelcomeMessage(sessionType, context) {
        const welcomeMessages = {
            [AITutorSession_1.SessionType.GENERAL_QA]: '안녕하세요! TEPS 학습을 도와드릴 AI 튜터입니다. 무엇을 도와드릴까요?',
            [AITutorSession_1.SessionType.PROBLEM_EXPLANATION]: '문제 해설을 도와드리겠습니다. 어떤 문제가 궁금하신가요?',
            [AITutorSession_1.SessionType.STUDY_COACHING]: '학습 전략에 대해 상담해드리겠습니다. 현재 고민하시는 부분을 말씀해주세요.',
            [AITutorSession_1.SessionType.MOTIVATION]: '힘든 순간이신가요? 함께 이겨내봅시다! 어떤 점이 가장 어려우신가요?',
            [AITutorSession_1.SessionType.GOAL_SETTING]: '목표를 함께 설정해봅시다. 원하시는 TEPS 점수와 목표 달성 시기를 알려주세요.',
            [AITutorSession_1.SessionType.PROGRESS_REVIEW]: '지금까지의 학습 진도를 리뷰해드리겠습니다. 궁금하신 점이 있으신가요?',
        };
     ...
     */
    static async chat(sessionId, userMessage) {
        const session = await AITutorSession_1.AITutorSession.findById(sessionId);
        ...
        recentMessages.forEach((msg) => {
            if (msg.role !== AITutorSession_1.MessageRole.SYSTEM) {
                messages.push({
                    role: msg.role,
                    content: msg.content,
                });
        ...
        try {
            ...
            await session.addMessage(AITutorSession_1.MessageRole.USER, userMessage, { sentimentScore });
            await session.addMessage(AITutorSession_1.MessageRole.ASSISTANT, assistantMessage);
            ...
            const recommendedActions = this.generateRecommendedActions(session.context, sentimentScore);
            // Add motivational boost if user is struggling
            let motivationalBoost;
            if (sentimentScore < -0.3 || session.context?.mood === 'frustrated') {
                motivationalBoost = await this.generateMotivationalBoost(session.userId.toString());
            }
            return {
     ...
     */
    static getSystemPrompt(sessionType, context) {
...
- Balancing different TEPS sections`,
            [AITutorSession_1.SessionType.MOTIVATION]: `\n\nFocus on:
- Acknowledging struggles empathetically
- Celebrating small wins
- Reframing challenges as opportunities
- Sharing success stories
- Building confidence`,
            [AITutorSession_1.SessionType.GOAL_SETTING]: `\n\nFocus on:
- Setting SMART goals (Specific, Measurable, Achievable, Relevant, Time-bound)
- Breaking down long-term goals into milestones
- Creating accountability systems
- Tracking progress metrics`,
            [AITutorSession_1.SessionType.PROGRESS_REVIEW]: `\n\nFocus on:
...
- Adjusting study plans based on data`,
     ...
     */
    static extractSuggestions(message) {
        const suggestions = [];
     ...
     */
    static extractRelatedTopics(message) {
     ...
     */
    static generateRecommendedActions(context, sentimentScore) {
        const actions = [];
     ...
     */
    static async generateMotivationalBoost(userId) {
     ...
     * End session
     */
    static async endSession(sessionId, userFeedback) {
        const session = await AITutorSession_1.AITutorSession.findById(sessionId);
        if (!session) {
    ...
    static async generateWeeklyReport(userId) {
        ...
        const recentExams = await TEPSExam_1.TEPSExamAttempt.find({
            userId: new mongoose_1.default.Types.ObjectId(userId),
            completedAt: { $gte: oneWeekAgo },
        }).sort({ completedAt: -1 });
        // Get recent tutor sessions
        const recentSessions = await AITutorSession_1.AITutorSession.find({
            userId: new mongoose_1.default.Types.ObjectId(userId),
            startedAt: { $gte: oneWeekAgo },
        });
        // Calculate metrics
        const studyDays = new Set([...recentExams.map((e) => e.completedAt?.toISOString().split('T')[0])].filter(Boolean)).size;
        const questionsAttempted = profile?.performanceHistory.filter((p) => p.timestamp >= oneWeekAgo).length || 0;
        const recentPerformance = profile?.performanceHistory.filter((p) => p.timestamp >= oneWeekAgo) || [];
        ...
        return {
            ...
            challenges: accuracy < 60 ? ['정답률이 목표치(70%)보다 낮습니다.'] : [],
            recommendations,
            actionPlan: [
                '매일 30분 이상 학습 시간 확보',
                '취약 영역 집중 학습',
     ...
     */
    static async createCoachingSession(userId, coachingType) {
        let performanceSummary;
        let insights = [];
        let recommendations = [];
        if (coachingType === LearningCoachSession_1.CoachingType.WEEKLY_CHECKIN) {
            ...
            performanceSummary = {
                ...
                questionsAttempted: parseInt(report.summary.match(/(\d+)문제 풀이/)?.[1] || '0'),
                accuracy: parseFloat(report.summary.match(/([\d.]+)% 정답률/)?.[1] || '0'),
                scoreChange: 0,
        ...
        const session = await LearningCoachSession_1.LearningCoachSession.create({
            userId: new mongoose_1.default.Types.ObjectId(userId),
            coachingType,
            insights,
            recommendations,
     ...
     * Add learning habit
     */
    static async addLearningHabit(userId, habitName, description, frequency, targetDays, targetTime) {
        const session = await LearningCoachSession_1.LearningCoachSession.findOne({
            userId: new mongoose_1.default.Types.ObjectId(userId),
        }).sort({ createdAt: -1 });
        if (!session) {
            // Create a new coaching session
            const newSession = await LearningCoachSession_1.LearningCoachSession.create({
                userId: new mongoose_1.default.Types.ObjectId(userId),
                coachingType: LearningCoachSession_1.CoachingType.HABIT_BUILDING,
            });
     ...
     */
    static async sendDailyMotivation(userId) {
        const profile = await UserLearningProfile_1.UserLearningProfile.findOne({
            userId: new mongoose_1.default.Types.ObjectId(userId),
        });
        let message = '';
...
exports.EnhancedAITutorService = EnhancedAITutorService;
//# sourceMappingURL=enhancedAITutorService.js.map