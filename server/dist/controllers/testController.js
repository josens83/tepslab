...
exports.createTest = exports.getMyTestResults = exports.getTestResult = exports.submitTest = exports.getTestById = exports.getTests = void 0;
...
const getTests = async (req, res) => {
    try {
        ...
        if (targetScore)
            query.targetScore = Number(targetScore);
        const tests = await Test_1.Test.find(query)
...
const submitTest = async (req, res) => {
    try {
        ...
        // Save result
        const testResult = await TestResult_1.TestResult.create({
            userId,
            testId: id,
...
const getTestResult = async (req, res) => {
    try {
        ...
        const result = await TestResult_1.TestResult.findById(id).populate('testId', 'title description targetScore');
        if (!result) {
        ...
        const test = await Test_1.Test.findById(result.testId);
...
const getMyTestResults = async (req, res) => {
    try {
        ...
        const results = await TestResult_1.TestResult.find({ userId })
...
exports.getMyTestResults = getMyTestResults;
/**
 * 추정 TEPS 점수 계산 (간소화된 공식)
 */
function calculateEstimatedScore(percentage, targetScore) {
...
const createTest = async (req, res) => {
    try {
        const { title, description, testType, targetScore, duration, questions, passingScore, } = req.body;
        if (!title || !description || !questions || questions.length === 0) {
            res.status(400).json({ error: '필수 항목을 입력해주세요.' });
            return;
        }
        const test = await Test_1.Test.create({
            title,
            description,
