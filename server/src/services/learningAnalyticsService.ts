import { LearningAnalytics, ILearningAnalytics } from '../models/LearningAnalytics';
...
export interface DashboardData {
  ...
  studyPatterns: {
    weekdayDistribution: { day: string; hours: number; productivity: number }[];
...
export class LearningAnalyticsService {
   ...
   */
  static async getOrCreateAnalytics(userId: string): Promise<ILearningAnalytics> {
   ...
   */
  static async updateAnalytics(userId: string): Promise<ILearningAnalytics> {
    ...
    const exams = await TEPSExamAttempt.find({
      userId: new mongoose.Types.ObjectId(userId),
      status: 'completed',
    })
      .sort({ completedAt: -1 })
    ...
    if (profile) {
      const sectionPerformance = Object.values(TEPSSection).map((section) => {
        const sectionHistory = profile.performanceHistory.filter(
          (p) => p.section === section
        );
      ...
      analytics.strongestSection = sectionPerformance[0]?.section || TEPSSection.LISTENING;
      analytics.weakestSection =
        sectionPerformance[sectionPerformance.length - 1]?.section || TEPSSection.READING;
      ...
      const weekdayStats: Record<string, { hours: number; correct: number; total: number }> = {
      ...
      profile.performanceHistory.forEach((p) => {
        ...
        if (weekdayStats[dayName]) {
          ...
          if (p.isCorrect) weekdayStats[dayName].correct += 1;
        }
      });
      ...
      const sortedByProductivity = [...analytics.studyTimeDistribution].sort(
        (a, b) => b.productivity - a.productivity
      );
      analytics.mostProductiveTime = sortedByProductivity[0]?.dayOfWeek || 'Monday';

      // Calculate learning velocity
      const last7Days = profile.performanceHistory.filter(
        (p) => p.timestamp >= new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
      );
      const questionsPerDay = last7Days.length / 7;

      const last14Days = profile.performanceHistory.filter(
        (p) => p.timestamp >= new Date(Date.now() - 14 * 24 * 60 * 60 * 1000)
      );
      const firstWeek = last14Days.filter(
        (p) => p.timestamp < new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
      );
   ...
   */
  static async generateDashboard(userId: string): Promise<DashboardData> {
    ...
    if (profile && profile.performanceHistory.length > 0) {
      ...
      while (!foundGap && studyStreak < 365) {
        ...
        const hasActivity = profile.performanceHistory.some(
    ...
    const last30Days = analytics.scoreTrends.filter(
    ...
    const nullArray: (number | null)[] = Array(predictionDates.length).fill(null);
    const nullArray2: (number | null)[] = Array(dates.length).fill(null);
  ...
  static async checkMilestones(userId: string): Promise<void> {
    ...
    questionMilestones.forEach((count) => {
      if (
        analytics.totalQuestionsAttempted >= count &&
