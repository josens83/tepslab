...
export interface AdaptiveSelectionCriteria {
  userId: string;
  targetScore: number;
  currentAbility: number; // IRT theta parameter
  weakTopics: string[];
  recentQuestions: string[]; // Question IDs to avoid
 ...
 */
export class TEPSQuestionBankService {
  /**
   ...
   */
  private static buildGenerationPrompt(request: QuestionGenerationRequest): string {
    ...
    if (section === TEPSSection.LISTENING) {
    ...
    } else if (section === TEPSSection.READING) {
    ...
    } else if (section === TEPSSection.VOCABULARY) {
    ...
    } else if (section === TEPSSection.GRAMMAR) {
   ...
   */
  private static getSystemPromptForSection(section: TEPSSection): string {
    ...
    const sectionPrompts: Record<TEPSSection, string> = {
      [TEPSSection.LISTENING]: `${basePrompt} You specialize in creating listening comprehension questions that test understanding of conversations, talks, and lectures in academic and professional contexts.`,
      [TEPSSection.VOCABULARY]: `${basePrompt} You specialize in creating vocabulary questions that test knowledge of academic and professional English words, including definitions, synonyms, and contextual usage.`,
      [TEPSSection.GRAMMAR]: `${basePrompt} You specialize in creating grammar questions that test understanding of English grammar rules, sentence structure, and error identification commonly tested in TEPS.`,
      [TEPSSection.READING]: `${basePrompt} You specialize in creating reading comprehension questions based on academic and professional passages, testing main ideas, details, inferences, and vocabulary in context.`,
    };
   ...
   */
  private static async createQuestionFromAI(
    data: any,
    request: QuestionGenerationRequest
  ): Promise<ITEPSQuestion> {
    const questionData: Partial<ITEPSQuestion> = {
    ...
    if (request.section === TEPSSection.LISTENING && data.audioTranscript) {
    ...
    if (request.section === TEPSSection.READING && data.passageContent) {
    ...
    const question = await TEPSQuestion.create(questionData);
    return question;
  }
  ...
  static async searchQuestions(
    ...
    limit: number = 50,
    skip: number = 0
  ): Promise<{ questions: ITEPSQuestion[]; total: number }> {
    const query: any = {};
    ...
    const [questions, total] = await Promise.all([
      TEPSQuestion.find(query).limit(limit).skip(skip).sort({ createdAt: -1 }).exec(),
      TEPSQuestion.countDocuments(query),
    ]);
  ...
  static async selectAdaptiveQuestions(
    criteria: AdaptiveSelectionCriteria,
    count: number = 10
  ): Promise<ITEPSQuestion[]> {
    ...
    const candidates = await TEPSQuestion.find(query)
      .limit(count * 3)
      .exec();
   ...
   */
  private static abilityToDifficulty(ability: number): DifficultyLevel {
    // ability ranges from -3 to +3
    // Map to difficulty 1-5
    if (ability <= -2) return DifficultyLevel.VERY_EASY;
    if (ability <= -0.5) return DifficultyLevel.EASY;
    if (ability <= 0.5) return DifficultyLevel.MEDIUM;
    if (ability <= 2) return DifficultyLevel.HARD;
   ...
   */
  static async getQuestionBankStats(): Promise<{
    ...
    const [
      ...
      TEPSQuestion.countDocuments({ reviewStatus: 'approved' }),
      TEPSQuestion.aggregate([
      ...
      TEPSQuestion.countDocuments({ reviewStatus: 'approved', isOfficialQuestion: true }),
      TEPSQuestion.countDocuments({ reviewStatus: 'approved', isAIGenerated: true }),
      TEPSQuestion.aggregate([
        { $match: { reviewStatus: 'approved' } },
   ...
   */
  static async analyzeOfficialPatterns(section: TEPSSection): Promise<{
    ...
    const officialQuestions = await TEPSQuestion.find({
      section,
    ...
    const totalLength = officialQuestions.reduce(
      (sum, q) => sum + q.questionText.length,
      0
    );
  ...
  static async bulkImportQuestions(
    questions: Partial<ITEPSQuestion>[],
    ...
    for (const questionData of questions) {
      try {
        ...
        await TEPSQuestion.create(questionData);
        imported++;
      } catch (error: any) {
        failed++;
