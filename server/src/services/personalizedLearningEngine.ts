...
import { TEPSQuestionBankService, AdaptiveSelectionCriteria } from './tepsQuestionBankService';
...
export interface UserLearningProfile {
  ...
  // IRT Ability Estimates (theta parameter for each section)
  abilityEstimates: {
    listening: number; // -3 to +3
    vocabulary: number;
    grammar: number;
    reading: number;
    overall: number;
  ...
  // Learning Patterns
  learningPatterns: {
    optimalStudyTime: string; // Time of day (e.g., "morning", "afternoon", "evening")
...
export interface StudyPlan {
  ...
  weeklyPlan: {
    ...
    expectedProgress: number; // Expected score increase
  }[];
...
export class PersonalizedLearningEngine {
   ...
   */
  static async initializeProfile(userId: string): Promise<UserLearningProfile> {
   ...
   */
  static async updateProfileWithResponse(
    ...
    profile.abilityEstimates[section] = this.estimateAbilityIRT(
      profile.abilityEstimates[section],
      question,
      isCorrect
    );
   ...
   */
  private static async updateTopicPerformance(
    profile: UserLearningProfile,
    question: ITEPSQuestion,
    isCorrect: boolean
  ): Promise<void> {
    ...
    // Update weak topics
    const weakTopicIndex = profile.weakTopics.findIndex(
      (wt) => wt.section === section && wt.topic === topic
    );
    ...
    const strongTopicIndex = profile.strongTopics.findIndex(
      (st) => st.section === section && st.topic === topic
    );
   ...
   * Update learning patterns
   */
  private static async updateLearningPatterns(profile: UserLearningProfile): Promise<void> {
    ...
    Object.keys(performanceByTime).forEach((time) => {
      const count = timeDistribution[time as keyof typeof timeDistribution];
      if (count >= 5) {
        const rate = performanceByTime[time as keyof typeof performanceByTime] / count;
        if (rate > bestRate) {
          bestRate = rate;
          bestTime = time;
        }
    ...
    const sessionDurations = Array.from(sessionsByDate.values()).map((times) =>
      times.reduce((sum, t) => sum + t, 0)
    );
    const avgDuration =
      sessionDurations.reduce((sum, d) => sum + d, 0) / sessionDurations.length / 60;
    ...
    if (improvement > 0.5) profile.learningPatterns.learningSpeed = 'fast';
    else if (improvement > 0.2) profile.learningPatterns.learningSpeed = 'average';
    else profile.learningPatterns.learningSpeed = 'slow';

    // Calculate consistency score (based on regular study)
    const studyDates = new Set(
      recentHistory.map((h) => h.timestamp.toISOString().split('T')[0])
    );
  ...
  private static calculateAverageAbility(
    history: UserLearningProfile['performanceHistory']
  ): number {
  ...
  private static determineFocusSections(
    ...
    const sectionAbilities = [
      { section: TEPSSection.LISTENING, ability: profile.abilityEstimates.listening },
      { section: TEPSSection.VOCABULARY, ability: profile.abilityEstimates.vocabulary },
      { section: TEPSSection.GRAMMAR, ability: profile.abilityEstimates.grammar },
      { section: TEPSSection.READING, ability: profile.abilityEstimates.reading },
    ];
    ...
    if (week % 3 === 0) {
      ...
      return [
        TEPSSection.LISTENING,
        TEPSSection.VOCABULARY,
        TEPSSection.GRAMMAR,
        TEPSSection.READING,
      ];
    } else if (week % 2 === 0) {
  ...
  private static generateDailySessions(
    profile: UserLearningProfile,
    focusSections: TEPSSection[],
    weeklyIncrease: number
  ): StudyPlan['weeklyPlan'][0]['dailySessions'] {
    const days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
    const sessions: StudyPlan['weeklyPlan'][0]['dailySessions'] = [];
  ...
  private static async generateRecommendations(
      ...
      const questions = await TEPSQuestionBankService.searchQuestions(
        {
          ...
          difficultyLevel: [DifficultyLevel.HARD, DifficultyLevel.VERY_HARD],
        },
        10
      );
    ...
    const recentSections = new Set(
      profile.performanceHistory.slice(-50).map((h) => h.section)
    );
    const allSections = Object.values(TEPSSection);
   ...
   */
  private static abilityToScore(ability: number): number {
   ...
   */
  static async getNextQuestion(profile: UserLearningProfile): Promise<ITEPSQuestion | null> {
    ...
    let targetSection: TEPSSection;
    if (profile.weakTopics.length > 0) {
      targetSection = profile.weakTopics[0].section;
    } else {
      // Rotate through sections
      const sections = Object.values(TEPSSection);
      ...
      targetSection = sections[(lastIndex + 1) % sections.length] || TEPSSection.LISTENING;
    }

    const criteria: AdaptiveSelectionCriteria = {
      userId: profile.userId,
    ...
    const questions = await TEPSQuestionBankService.selectAdaptiveQuestions(criteria, 1);
